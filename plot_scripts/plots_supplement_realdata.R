#Plots of real data example for Supplement
...
plots_pred <- lapply(res_pred[, unique(n)], function(nm) {
  ggplot(res_pred[n == nm, ], aes(x = Method, y = perf,fill = Method, color = Method)) +
    ...
    ggtitle("a) Brier score - Diabetes dataset")+
    ylab("Brier score") + 
    xlab(NULL) +
    coord_flip()
})
names(plots_pred) <- res_pred[, unique(n)]
p1 <- patchwork::wrap_plots(plots_pred, ncol = 1)
...
plot_aw <- lapply(res[, unique(n)], function(nm) {
  ggplot(eva[n == nm, ], aes(x = Method, y = average_width, fill = Method, color=Method)) +
    facet_grid(prop_mis ~ pattern, labeller = labeller(prop_mis =c('0.1'=paste("mis. = 0.1"), '0.2'=paste("mis. = 0.2"), '0.4'=paste("mis. = 0.4")))) +
    geom_boxplot(outliers = FALSE) +
    theme_bw(base_size = 14) + 
    theme(plot.title = element_text(size = 14), axis.text.x = element_text(angle = 45, hjust = 1))+   
    ggtitle("b) Average CI width - Diabetes dataset")+
    coord_flip() + 
...
names(plot_aw) <- res[, unique(n)]

p2 <- wrap_plots(plot_aw, ncol = 1)

# Plot RMSE --------------------------------------------------------------------
plot_rmse <- lapply(res[, unique(n)], function(nm) {
  ggplot(eva[n == nm, ], aes(x = Method, y = rmse, fill = Method, color=Method)) +
    facet_grid(prop_mis ~ pattern, labeller = labeller(prop_mis =c('0.1'=paste("mis. = 0.1"), '0.2'=paste("mis. = 0.2"), '0.4'=paste("mis. = 0.4")))) +
    geom_boxplot(outliers = FALSE) +
    theme_bw(base_size = 14) + 
    theme(plot.title = element_text(size = 14), axis.text.x = element_text(angle = 45, hjust = 1))+   
    ggtitle("c) Coefficient RMSE - Diabetes dataset")+
    coord_flip() + 
...
names(plot_rmse) <- res[, unique(n)]

p3 <- wrap_plots(plot_rmse, ncol = 1)

## Combine all plots and save ---------------------------------------------------------
bb <- (p1 | p2)/(p3|plot_spacer()) 

ggsave("supplement_plots/plot_real_pred_rmse_aw.pdf", plot = bb, width = 210, height = 173.33, units = "mm", scale = 1.5)
